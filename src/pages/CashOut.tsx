...
export default function CashOut() {
  ...
  const [formData, setFormData] = useState({
    ...
    amount: '',
    category: categories_expense[0] || 'Misc',
    description: '',
  ...
  // Query Cash Out Transactions (excluding specific purchase entries so general withdrawals/bills are managed)
  const cashOutTransactions = useLiveQuery(async () => {
  ...
  const handleAddCashOut = async (e: React.FormEvent) => {
    ...
    try {
      // 1. First create linked entry in expenses table
      ...
      toast.success('Cash Out Recorded', `Amount of ${formatCurrency(amountNum, currencySymbol, currency)} deducted successfully.`);
      ...
      console.error(err);
      toast.error('Operation Failed', 'Could not record cash out. Try again.');
    }
  };

  // Handle Row Deletion (with Cascade delete on linked expense record)
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      if (tx) {
        if (tx.linkId) {
          await db.expenses.delete(tx.linkId);
        }
        await db.transactions.delete(id);
      }
      toast.success('Transaction Deleted', 'The cash-out entry has been removed.');
    } catch (err) {
      console.error(err);
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Update linked expense if linkId exists
      if (editingTx.linkId) {
        await db.expenses.update(editingTx.linkId, {
          date: editFormData.date,
          amount: amountNum,
          category: editFormData.category,
          description: editFormData.description,
  ...
  const filteredData = cashOutTransactions?.filter((t) => {
    const matchesSearch =
  ...
  const handleExportExcel = () => {
    ...
    exportToExcel(exportData, 'Cash_Out_Report');
  };
  ...
  const handleExportCSV = () => {
    ...
    exportToCSV(exportData, 'Cash_Out_Report');
  };

  return (
    <div className="space-y-6" id="cash-out-view">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-1">
            Log overheads, utilities, salaries, and other non-inventory cash outflows here.
          </p>
        </div>
        <button
          onClick={() => setShowAddModal(true)}
          className="inline-flex items-center gap-2 bg-rose-600 hover:bg-rose-700 text-white text-sm font-semibold px-4 py-2.5 rounded-xl transition-colors cursor-pointer self-start sm:self-auto"
        >
      ...
      <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-4 shadow-sm flex flex-col md:flex-row gap-4 items-center">
        ...
        <div className="relative w-full md:flex-1">
          ...
          <input
            ...
            className="w-full bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl pl-10 pr-4 py-2 text-sm text-zinc-900 dark:text-zinc-100 placeholder-zinc-400 focus:outline-none focus:ring-2 focus:ring-rose-500"
          />
        </div>
        ...
        <div className="flex flex-wrap gap-3 w-full md:w-auto">
          ...
          <div className="flex items-center gap-1.5 border-l border-zinc-200 dark:border-zinc-800 pl-3">
            <button
              ...
              className="p-1.5 text-zinc-500 hover:text-rose-600 hover:bg-zinc-50 dark:hover:bg-zinc-800 rounded-lg disabled:opacity-50 cursor-pointer"
              ...
              {filteredData.length === 0 ? (
                <tr>
                  <td colSpan={7} className="text-center p-12 text-zinc-400">
                    ...
                    <p className="text-xs mt-1">Try resetting filters or log a new Cash Out entry.</p>
                  </td>
                </tr>
              ) : (
                filteredData.map((t) => (
                  <tr key={t.id} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-800/20 transition-colors">
                    ...
                    <td className="p-4 whitespace-nowrap">
                      <span className="inline-flex items-center gap-1 px-2.5 py-1 bg-rose-50 dark:bg-rose-950/40 text-rose-700 dark:text-rose-400 rounded-full text-xs font-medium">
                        {t.category}
                      </span>
                    </td>
          ...
          <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-2xl max-w-lg w-full overflow-hidden flex flex-col">
            ...
            <div className="p-5 border-b border-zinc-100 dark:border-zinc-800 flex items-center justify-between">
              <h3 className="text-lg font-bold text-zinc-900 dark:text-zinc-100 flex items-center gap-2">
                ...
                <span>Record Cash Out Outflow</span>
              </h3>
              <button
            ...
            {/* Modal Form */}
            <form onSubmit={handleAddCashOut} className="p-5 space-y-4 overflow-y-auto max-h-[75vh]">
              {/* Grid 1: Date & Amount */}
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    className="w-full bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl px-3 py-2 text-sm text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-rose-500"
                  />
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    className="w-full bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl px-3 py-2 text-sm text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-rose-500 font-semibold"
                  />
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  ...
                  className="w-full bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl px-3 py-2 text-sm text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-rose-500 resize-none"
                />
              </div>
              ...
              <div className="pt-4 border-t border-zinc-100 dark:border-zinc-800 flex justify-end gap-3">
                ...
                <button
                  type="submit"
                  className="px-5 py-2 bg-rose-600 hover:bg-rose-700 text-white rounded-xl text-sm font-semibold transition-colors cursor-pointer shadow-sm"
                >
                  Save Outflow
                </button>
              </div>
          ...
          <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-2xl max-w-lg w-full overflow-hidden flex flex-col">
            ...
            <div className="p-5 border-b border-zinc-100 dark:border-zinc-800 flex items-center justify-between">
              <h3 className="text-lg font-bold text-zinc-900 dark:text-zinc-100 flex items-center gap-2">
                ...
                <span>Edit Cash Outflow Entry</span>
              </h3>
              <button
                onClick={() => {
                  setShowEditModal(false);
